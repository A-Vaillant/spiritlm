...
# Add spiritlm to path
sys.path.append('/app/spiritlm')
# Configure logging to output to stdout (Docker captures this)
...
# Set the SpiritLM logger specifically
spiritlm_logger = logging.getLogger('spiritlm')
spiritlm_logger.setLevel(level=getattr(logging, log_level, logging.INFO))

# Initialize the Spirit LM base model. You can change this to spirit-lm-expressive-7b for a more expressive experience.
model_type = 'spirit-lm-base-7b'
print(f"Initializing {model_type} on {device}...")
spirit_lm = Spiritlm(model_type, parallel_style=None)
